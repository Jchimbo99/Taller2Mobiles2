...
async function guardarCambios() {
  ...
  try {
    let nuevaUrl = avatarUrl;


    if (image) {

      if (avatarUrl && avatarUrl.includes('/storage/v1/object/public/registro/')) {
        const path = avatarUrl.split('/storage/v1/object/public/registro/')[1];
        const { error: deleteError } = await supabase.storage
          .from('registro')
          .remove([`public/${path}`]);

        if (deleteError) {
          console.warn('⚠️ No se pudo eliminar avatar anterior:', deleteError.message);
        }
      }
